  ...
  return (
    <div className="pz-bg-white pz-rounded-xl pz-shadow-md pz-overflow-hidden pz-border-2 pz-border-emerald-500">
      ...
      <div className="pz-p-4 pz-border-b pz-border-gray-100">
        <h4 className="pz-text-2xl pz-font-bold pz-mb-2">{order.fullName}</h4>

        {/* Phone - Large and tappable */}
        <button
          onClick={handleCall}
          className="pz-flex pz-items-center pz-gap-3 pz-w-full pz-p-3 pz-bg-blue-50 pz-rounded-lg pz-mb-3 hover:pz-bg-blue-100 pz-transition"
        >
          <Phone className="pz-text-blue-600" size={24} />
          <span className="pz-text-xl pz-font-semibold pz-text-blue-600">{order.phone}</span>
        </button>

        {/* Address - Large and tappable */}
        <button
          onClick={handleOpenMaps}
          className="pz-flex pz-items-start pz-gap-3 pz-w-full pz-p-3 pz-bg-green-50 pz-rounded-lg hover:pz-bg-green-100 pz-transition pz-text-left"
        >
          <MapPin className="pz-text-green-600 pz-mt-1" size={24} />
          <div>
            <span className="pz-text-lg pz-font-semibold">{order.address}</span>
            <div className="pz-text-sm pz-text-green-600 pz-flex pz-items-center pz-gap-1 pz-mt-1">
              <Navigation size={14} />
              Open in Maps
            </div>
          </div>
      ...
      {/* Order Notes */}
      {order.comment && (
        <div className="pz-px-4 pz-py-3 pz-bg-yellow-50 pz-border-b pz-border-gray-100">
      ...
      {/* Order Items Summary */}
      <div className="pz-p-4 pz-border-b pz-border-gray-100">
        <h5 className="pz-font-semibold pz-mb-2">Order Items ({items.length})</h5>
        <ul className="pz-text-sm pz-text-gray-600">
          {items.map((item, idx) => (
            <li key={idx}>
        ...
        <div className="pz-mt-2 pz-text-lg pz-font-bold">
          Total: {order.totalAmount} ILS
        </div>
      </div>

      {/* Action Buttons */}
      <div className="pz-p-4 pz-space-y-3">
        <div className="pz-flex pz-gap-3">
          <Button
            variant="outline"
            onClick={handleCall}
            className="pz-flex-1 pz-h-14"
          >
            <Phone size={20} className="pz-mr-2" />
            Call
          </Button>
          <Button
            variant="outline"
            ...
            className="pz-flex-1 pz-h-14"
          >
            <Navigation size={20} className="pz-mr-2" />
            Navigate
          </Button>
        </div>
        <Button
          ...
          disabled={loading}
          className="pz-w-full pz-h-16 pz-text-lg pz-bg-emerald-500 hover:pz-bg-emerald-600"
        >
          <CheckCircle size={24} className="pz-mr-2" />
